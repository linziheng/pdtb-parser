...
public class Tester {
  ...
  private static void testArgExt(boolean train) throws IOException {
    ...
    for (FeatureType featureType : FeatureType.testingValues()) {
      ...
      log.info("\nEXACT \nA1: " + score[0].print(score[0].f1) + "\tA2: "
          + score[1].print(score[1].f1) + "\tA1A2: " + score[2].print(score[2].f1));
  ...
  public static void testConn(boolean train) throws IOException {
    ...
    File autoResultFile = new File(Settings.OUT_PATH + ConnComp.NAME + FeatureType.Auto + ".out");
    res = Scorer.conn(conn.getGsFile(), autoResultFile);
    log.info("Auto");
    log.info("Acc:" + res.print(res.acc));
    log.info("F1:" + res.print(res.f1));
  }
